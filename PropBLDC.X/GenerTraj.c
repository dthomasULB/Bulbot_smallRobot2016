...
//								CONSTANTS									//
//////////////////////////////////////////////////////////////////////////////
#include "test2_2016.h"     //définit initialMap
...
inline mapPosType RPtoMP(absolutePosType rpPos) {
    ...
    mpPos.a = (rpPos.x - ORIG_X)*10 ;/// DAX;
    mpPos.b = (rpPos.y - ORIG_Y)*10 ;/// DBY;
    if (rpPos.alpha >= 0) {
...
inline int findPath(mapPosType mpStart, mapPosType mpEnd, mapPosType mpPath[]) {
    ...
    if ( (dkNodes[MAP_SIZE_B * mpStart.a + mpStart.b].done == OBST) ||
        ...
        dkNodes[MAP_SIZE_B * mpStart.a + mpStart.b].done = UNPR;                    // On le retire sinon on reste bloqué, la logique nous fait reculer pour en "sortir" 
        return START_OBST;
    } else if ( (dkNodes[MAP_SIZE_B * mpEnd.a + mpEnd.b].done == OBST) ||
                (dkNodes[MAP_SIZE_B * mpEnd.a + mpEnd.b].done == MOVE_OBST) ) {
        dkNodes[MAP_SIZE_B * mpStart.a + mpStart.b].done = UNPR; 
        return END_OBST;
    }
...
propIsObstacleType trajIsObstacleInMap(obstacleType obstacle){//(absolutePosType rpPoint) {
 ...
 //   mpPoint = RPtoMP(rpPoint);
    if ((dkNodes[MAP_SIZE_B * mpPoint.a + mpPoint.b].done == OBST || 30 < mpPoint.a) || 20 < mpPoint.b) {
        return (PROP_IS_UNMOVABLE_OBSTACLE);
    } else if (dkNodes[MAP_SIZE_B * mpPoint.a + mpPoint.b].done == MOVE_OBST){
...
int findTrajectoire(absolutePosType rpStart, absolutePosType rpEnd, absolutePosType rpPath[]) {
    ...
    if (250 > trajLength && trajLength > 1 ) {                                                       // Si une trajectoire a été trouvée (au moins 2 points),
