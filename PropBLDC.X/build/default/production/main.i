...
void __attribute__((interrupt, auto_psv)) _T1Interrupt(void){
    ...
    canSendPos(odoGetAbsPos());
    calculeOdometrie();
    if (isrRegFlag) {
        csgCompute();
        patinageFlag = regCompute();
    }
    (LATBbits.LATB5) = 0;
}
...
int main(void) {
    propStateType state = DISABLED;
    propStateType oldState = DISABLED;
    csgStatusType csgStatus;
    relativePosType nomVel;
    relativePosType nomAcc;
    absolutePosType curPos;

    translationParamType translationParam;
    rotationParamType rotationParam;

    absolutePosType finalPos;
    relativePosType segment;
    absolutePosType path[15];
    propIsObstacleType obstacle;

    int patinageEnable = 0;
    int backward = 0;
    int curSeg, nbSeg;
    enum {
        XY,
    ...
    while (1) {
        switch (state) {
                ...
                if (canReceivedOrderFlag) {
                    switch (canReceivedCommand) {
                        ...
                        case 0x12:
                            ...
                            break;
                        case 0x0D:
                            patinageEnable = (canReceivedData[0]);
                            break;
                        case 0x0E:
                ...
                switch (trajectorySubState) {
                    case ROTATION:
                        if (trajectorySubState != oldTrajectorySubState) {
                            ...
                            curPos = odoGetAbsPos();
                            IEC0bits.T1IE = 1;
                            segment = calcSegment(curPos, path[curSeg+1], backward);
                            if (segment.r != 0) {
            ...
            case TRAJ_NO_WAY:
                ...
                if ( (canReceivedOrderFlag) && (canReceivedCommand == 3)) {
                    state = STANDING;
                }
                break;
