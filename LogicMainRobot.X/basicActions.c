  ...
  infoActionType prendre(positionInteger posObj,technique method){
    ...
    switch (method){
        case (AUCUNE_ACTION):               // = Déposer
            rentrerServoClap();     // = Ouvrir pour poser
            infoAction.statut = ACTION_FINIE;
            break;
   ...
   infoActionType tirer(positionInteger posObj,technique method){
    ...
    switch (method){
        ...
        case (ACTIONNEUR):
            infoAction.statut = ACTION_EN_COURS;
            if (compareXYAlpha(posObj, propulsionGetPosition())){
                sortirServoClap(); // Fonction permettant le tir cabalistique
                infoAction.statut = ACTION_FINIE;
                break;
...
infoActionType gener(positionInteger destination,positionInteger destination2){
    // On ne le fait que si on a pas mieux, on va donc a des point où l'on pourrait géner notre adversaire/ protéger
    // On patrouille entre 2 positions
    static infoActionType infoAction = {ACTION_PAS_COMMENCEE, 0, 1, ACTION_TRAJECTOIRE, 0};
    infoAction.position = destination;
    if (compareXYAlpha(destination, propulsionGetPosition()) && infoAction.statut == ACTION_PAS_COMMENCEE){
    infoAction.position = destination2;
    }
    infoAction.statut = trajectoryBasicAction(infoAction.position);
    return (infoAction);
  }
 ...
 * // Test pour vérifier qu'on a bien l'objet
        ...
        case (ACTIONNEUR):
            infoAction.statut = ACTION_EN_COURS;
                sortirServoClap(); // action de stocker
                // Test pour vérifier qu'on a bien l'objet
